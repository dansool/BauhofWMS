...
[assembly: Dependency(typeof(ReadPurchaseReceiveRecords))]

namespace BauhofWMS.Droid.Utils
{
    public class ReadPurchaseReceiveRecords : IReadPurchaseReceiveRecordsAndroid
    {
        public async Task<string> ReadRecordsAsync(string shopLocationID, string deviceSerial)
        {
            ...
            try
            {
                ...
                {
                    if (r.Name.ToUpper().Contains("SHRCVDBRECORDS") && r.Name.ToUpper().EndsWith(".TXT"))
                    {
